...
function OrderDetails() {
  ...
  const handleSubmit = (e) => {
    ...
    const payload = {
      ...
      cmimi_unit: Number(form.cmimi_unit),
      zbritja: form.zbritja ? Number(form.zbritja) : 0,
    };
    const req = editId ? api.put(`/order-details/${editId}`, payload) : api.post('/order-details', payload);
    req.then(() => { setEditId(null); setForm(empty); fetchDetails(); }).catch(console.error);
  };
    ...
    <div>
        ...
        <>
          ...
          <form onSubmit={handleSubmit}>
            <select required value={form.order_id} onChange={set('order_id')}>
              ...
              {orders.map(o => <option key={o.id} value={String(o.id)}>Order #{o.id} — {o.customer_emri}</option>)}
            </select>
      ...
      {details.map(d => (
        <div key={d.id} className="list-row">
          ...
          <span>
            {canMutate && <button className="btn-edit" onClick={() => handleEdit(d)}>Edit</button>}
            {canDelete && <button className="btn-delete" onClick={() => handleDelete(d.id)}>Delete</button>}
          </span>
        </div>
      ))}
    </div>
