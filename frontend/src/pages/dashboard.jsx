    ...
    <div style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ marginBottom: '28px' }}>
        ...
        <p style={{ margin: 0, color: '#64748b', fontSize: '14px' }}>
          Pasqyra e sistemit — {new Date().toLocaleDateString('sq-AL', {
            weekday: 'long',
            year: 'numeric',
            month: 'long',
            day: 'numeric'
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginBottom: '28px' }}>
        ...
        <StatCard
          ...
          bg="#fef9c3"
          color="#a16207"
          sub="produkte nën 5 copë"
        />

        <StatCard
          label="Garanci Aktive"
          value={data.activeWarranties}
          icon={ShieldIcon}
          bg="#dbeafe"
          color="#1d4ed8"
          sub="në fuqi"
        ...
        <StatCard
          label="Servise Hapur"
          value={data.openServices}
          icon={WrenchIcon}
          bg="#fee2e2"
          color="#dc2626"
          sub="kërkesa të hapura"
        />
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '16px', marginBottom: '16px' }}>
        <div style={{ background: 'white', border: '1px solid #f1f5f9', borderRadius: '14px', padding: '20px 24px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
          <h2 style={{ margin: '0 0 20px', fontSize: '15px', fontWeight: '600', color: '#0f172a' }}>
            Të Ardhurat — 6 Muajt e Fundit
          </h2>
              ...
              {!data.recentOrders?.length ? (
                ...
                data.recentOrders.map(o => (
                  <tr key={o.id} style={{ borderTop: '1px solid #f8fafc' }}>
                    <td style={{ padding: '13px 20px', color: '#94a3b8', fontSize: '13px' }}>
                      #{o.id}
                    </td>

                    <td style={{ padding: '13px 20px', fontWeight: '500', color: '#0f172a', fontSize: '14px' }}>
                      {o.customer_emri || '—'}
                    </td>

                    <td style={{ padding: '13px 20px', fontWeight: '600', color: '#0f172a' }}>
                      {Number(o.totali).toLocaleString()}€
                    </td>

                    <td style={{ padding: '13px 20px' }}>
                      <Badge text={o.statusi} />
                    </td>
              ...
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '14px' }}>
                <div
                  style={{
                    ...
                    height: '28px',
                    borderRadius: '8px',
                    background: ['#dbeafe', '#dcfce7', '#fef9c3', '#ede9fe', '#fee2e2'][i],
                    display: 'flex',
                    alignItems: 'center',
                    justifyContent: 'center',
                    fontSize: '13px',
                    fontWeight: '700',
                    color: ['#1d4ed8', '#15803d', '#a16207', '#6d28d9', '#dc2626'][i],
                    flexShrink: 0
                  }}
                >
                  {i + 1}
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: '12px', color: '#94a3b8' }}>
                    {p.total_shitur} copë të shitura
                  </div>
                </div>
              </div>
            ))
