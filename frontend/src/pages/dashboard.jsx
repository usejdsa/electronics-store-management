  ...
  return (
    <div style={{ background: 'white', borderRadius: '14px', padding: '20px 24px', display: 'flex', alignItems: 'center', gap: '16px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)', border: '1px solid #f1f5f9' }}>
      <div style={{ width: '52px', height: '52px', borderRadius: '12px', background: bg, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
        <img src={icon} alt={label} style={{ width: '26px', height: '26px' }} />
      </div>
      <div>
...
function BarChart({ data }) {
  const { t } = useLang();
  if (!data || data.length === 0) return <div style={{ color: '#94a3b8', textAlign: 'center', padding: '40px' }}>{t.noData}</div>;
...
export default function Dashboard() {
  ...
  if (error) return <div style={{ background: '#fef2f2', border: '1px solid #fecaca', borderRadius: '12px', padding: '20px', color: '#dc2626' }}>{error}</div>;

  return (
    <div style={{ fontFamily: "'Inter', sans-serif" }}>
      <div style={{ marginBottom: '28px' }}>
        <h1 style={{ margin: '0 0 4px', fontSize: '24px', fontWeight: '700', color: '#0f172a' }}>{t.dashboardTitle}</h1>
        <p style={{ margin: 0, color: '#64748b', fontSize: '14px' }}>
          {t.dashboardSub} — {new Date().toLocaleDateString(lang === 'sq' ? 'sq-AL' : 'en-GB', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
        </p>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginBottom: '16px' }}>
        <StatCard label={t.stat_products} value={data.totalProducts} icon={BoxIcon} bg="#dbeafe" color="#1d4ed8" sub={t.stat_productsSub} />
        <StatCard label={t.stat_customers} value={data.totalCustomers} icon={UsersIcon} bg="#dcfce7" color="#15803d" sub={t.stat_customersSub} />
        <StatCard label={t.stat_orders} value={data.totalOrders} icon={CartIcon} bg="#fef9c3" color="#a16207" sub={t.stat_ordersSub} />
        <StatCard label={t.stat_suppliers} value={data.totalSuppliers} icon={FactoryIcon} bg="#ede9fe" color="#6d28d9" sub={t.stat_suppliersSub} />
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginBottom: '28px' }}>
        <StatCard label={t.stat_revenue} value={`${Number(data.totalRevenue).toLocaleString()}€`} icon={DollarIcon} bg="#dcfce7" color="#15803d" sub={t.stat_revenueSub} />
        <StatCard label={t.stat_lowStock} value={data.lowStock} icon={AlertIcon} bg="#fef9c3" color="#a16207" sub={t.stat_lowStockSub} />
        <StatCard label={t.stat_warranties} value={data.activeWarranties} icon={ShieldIcon} bg="#dbeafe" color="#1d4ed8" sub={t.stat_warrantiesSub} />
        <StatCard label={t.stat_services} value={data.openServices} icon={WrenchIcon} bg="#fee2e2" color="#dc2626" sub={t.stat_servicesSub} />
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '16px', marginBottom: '16px' }}>
        <div style={{ background: 'white', border: '1px solid #f1f5f9', borderRadius: '14px', padding: '20px 24px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
          <h2 style={{ margin: '0 0 20px', fontSize: '15px', fontWeight: '600', color: '#0f172a' }}>{t.chartRevenue}</h2>
          <BarChart data={data.revenueByMonth} />
        </div>
        <div style={{ background: 'white', border: '1px solid #f1f5f9', borderRadius: '14px', padding: '20px 24px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
          <h2 style={{ margin: '0 0 20px', fontSize: '15px', fontWeight: '600', color: '#0f172a' }}>{t.chartOrderStatus}</h2>
          {!data.ordersByStatus?.length
            ? <p style={{ color: '#94a3b8', fontSize: '14px' }}>{t.noOrders}</p>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '16px' }}>
        <div style={{ background: 'white', border: '1px solid #f1f5f9', borderRadius: '14px', overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
          <div style={{ padding: '18px 24px', borderBottom: '1px solid #f8fafc' }}>
            <h2 style={{ margin: 0, fontSize: '15px', fontWeight: '600', color: '#0f172a' }}>{t.recentOrders}</h2>
          </div>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: '#f8fafc' }}>
                {[t.colId, t.colClient, t.colTotal, t.colStatus, t.colDate].map(h => (
            ...
            <tbody>
              {!data.recentOrders?.length
                ? <tr><td colSpan="5" style={{ padding: '32px', textAlign: 'center', color: '#94a3b8' }}>{t.noOrders}</td></tr>
                : data.recentOrders.map(o => (
                  <tr key={o.id} style={{ borderTop: '1px solid #f8fafc' }}>
                    ...
                    <td style={{ padding: '13px 20px', color: '#94a3b8', fontSize: '13px' }}>{new Date(o.created_at).toLocaleDateString(lang === 'sq' ? 'sq-AL' : 'en-GB')}</td>
                  </tr>
                ))}
            </tbody>
          </table>
        ...
        <div style={{ background: 'white', border: '1px solid #f1f5f9', borderRadius: '14px', padding: '20px 24px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
          <h2 style={{ margin: '0 0 20px', fontSize: '15px', fontWeight: '600', color: '#0f172a' }}>{t.topProducts}</h2>
          {!data.topProducts?.length
            ? <p style={{ color: '#94a3b8', fontSize: '14px' }}>{t.noData}</p>
            : data.topProducts.map((p, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '14px' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: '12px', color: '#94a3b8' }}>{p.total_shitur} {t.unitsSold}</div>
                </div>
              </div>
            ))}
        </div>
